...
检查HTTP请求头中的 `Referer` 字段（该字段表示这个请求是从哪个页面链接过来的）。只允许来自您信任的域名（白名单）的请求访问资源，拒绝其他来源的请求。在CDN服务商的控制台中，设置Referer黑名单或白名单。

优点：
- 配置简单，能有效防止常见的站外盗链

缺点：
- **可绕过**：`Referer` 字段可以被恶意程序轻松伪造或直接置空。
- 某些合法场景（如从手机浏览器直接输入链接、某些App内请求）可能不携带 `Referer`，导致误杀。



#### 时间戳防盗链（URL 鉴权）

在提供给用户的 CDN 链接上，增加一个有时效性的**加密验证字符串**（通常包含时间戳、过期时间、MAC等参数）。**CDN 节点在收到请求时，会验证这个字符串的合法性和是否过期，并且如果不携带也会被认为是无效的**。

~~~bash
# 为一个原始资源URL生成一个带签名的临时URL。
# 原始链接 
https://cdn.example.com/video.mp4

#生成后的临时链接 
https://cdn.example.com/video.mp4?auth_key=timestamp-expiry-rand-md5hash
~~~

优点：
- **安全性高**：难以伪造，且链接有过期时间，极大增加了盗刷难度。
- **灵活控制**：可以精确控制单个链接的有效期（如1分钟、1小时、1天）。

缺点：
- 增加了您**服务器端的计算压力**（需要为每个请求生成签名）。
- 如果密钥泄露，整个机制将失效。
